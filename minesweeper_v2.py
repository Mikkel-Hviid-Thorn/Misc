...
def make_list(a,x,y): #make a 8x8 array
    l = []
    for i in range(0,x):
        l += [[]]
        for j in range(0,y):
...
def set_coordinates(x,y): #makes a 8x8 array with coordinat set for each element in the array
    l = []
    for i in range(0,x):
        for j in range(0,y):
            l += [[i,j]]
...
def neighbours(x,y): #makes a list of the neighbours for each element in a 8x8 array
    d = dict()
    for i in range(0,x):
        for j in range(0,y):
            d[i,j] = [] #there are 8 possible neighbours, therfore 8 conditions
            if i < x-1:
                d[i,j].append((i+1,j))
            if i > 0:
                d[i,j].append((i-1,j))
...
def place_bombs(x,y,antal): #places bombs in the 8x8 array and adds 1 to each neighbouring element
...
def clear(ms,b,m,n,x,y): #clears the neighbours of a element, if they meet certain requirement
    neighbour_dict = neighbours(x,y)
    ld,no_x = neighbour_dict[m,n],True
    #print(neighbour_dict)
    if b[m][n] == 0: #clears if element 0, therefore no bombs
        for i,j in ld:
            if b[i][j] != 'X' and ms[i][j] == '#':
                ms[i][j] = b[i][j]
                ms = clear(ms,b,i,j,x,y)
    elif b[m][n] in [1,2,3,4,5,6,7,8]: #checks if element integer has a neighbouring unknown bomb 
        for i,j in ld:
            if b[i][j] == 'X' and ms[i][j] == '#':
                no_x = False
    if no_x == True: #clears if element integer and has no neighbouring bomb
        for i,j in ld:
            if ms[i][j] == '#':
                ms[i][j] = b[i][j]
                ms = clear(ms,b,i,j,x,y)
...
def print_board(l,x,y): #prints the board for a 8x8 array
    print(' 0 |', end='')
...
def minesweeper(x,y,antal): #plays minesweeper / setup first
